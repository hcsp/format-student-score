...
public class Main {
    ...
    public static String formatStudentScore(String student, int score) {
        String format = "学生%s的分数是%d，评级是%c";
        if(score >= 90 && score <= 100){
            return String.format(format, student, score, 'A');
        }else if(score >= 60 && score <= 89) {
            return String.format(format, student, score, 'B');
        }else if(score >= 0 && score <= 59) {
            return String.format(format, student, score, 'C');
        }else{
            return "非法输入";
        }
    }
