...
public class Main {
    ...
    public static String formatStudentScore(String student, int score) {
        if (score >= 90 && score <= 100) {
            return String.format("学生%s的分数是%d，评级是A", student, score);
        } else if (score >= 60 && score <= 89) {
            return String.format("学生%s的分数是%d，评级是B", student, score);
        } else if (score >= 0 && score <= 59) {
            return String.format("学生%s的分数是%d，评级是C", student, score);
        } else {
            return "非法输入";
        }
    }
