...
public class Main {
    ...
    public static String formatStudentScore(String student, int score) {
        if (score >= 90 && score <= 100) {
            return String.format("学生%s的分数是%d，评级是%s", student, score, "A");
        }
        if (score >= 60 && score <= 89) {
            return String.format("学生%s的分数是%d，评级是%s", student, score, "B");
        }
        if (score >= 0 && score <= 59) {
            return String.format("学生%s的分数是%d，评级是%s", student, score, "C");
        }
        return "非法输入";
    }
