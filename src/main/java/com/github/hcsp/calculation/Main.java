...
public class Main {
    ...
    public static String formatStudentScore(String student, int score) {
        if(score >= 90 && score <= 100) {
            return new StringBuilder("学生").append(student).append("的分数是").append(score).append("，评级是A").toString();
        }

        if(score >= 60 && score <= 89) {
            return new StringBuilder("学生").append(student).append("的分数是").append(score).append("，评级是B").toString();
        }

        if(score >= 0 && score <= 59) {
            return new StringBuilder("学生").append(student).append("的分数是").append(score).append("，评级是C").toString();
        }
