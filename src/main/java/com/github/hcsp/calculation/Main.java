...
public class Main {
    ...
    public static String formatStudentScore(String student, int score) {
        if(score >= 90 && score <= 100) {
            return formatTemplate(student,score,"A");
        }else if(score >= 60 && score <= 89) {
            return formatTemplate(student,score,"B");
        }else if(score >= 0 && score <= 59) {
            return formatTemplate(student,score,"C");
        }else {
            return "非法输入";
        }
    }

    private static String formatTemplate(String student,int score,String level) {
        return String.format("学生%s的分数是%d，评级是%s",student,score,level);
    }
